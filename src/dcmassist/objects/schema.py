...
class SchemaPlugin(V1ObjectPlugin):
    ...
    MACRO_BODY = (
        "{% macro define_schema(database, name, transient=False, "
        "managed_access=False, data_retention_time_in_days=None, "
        "max_data_extension_time_in_days=None, default_ddl_collation=None, "
        "external_volume=None, catalog=None, log_level=None, trace_level=None, "
        "comment=None) %}\n"
        ...
        " EXTERNAL_VOLUME = '{{ external_volume }}'{% endif %}"
        "{% if catalog is not none %} CATALOG = '{{ catalog }}'{% endif %}"
        "{% if log_level is not none %} LOG_LEVEL = '{{ log_level }}'{% endif %}"
        "{% if trace_level is not none %} TRACE_LEVEL = '{{ trace_level }}'{% endif %}"
